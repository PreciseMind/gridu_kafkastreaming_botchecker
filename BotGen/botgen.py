...
user_categories = bot_categories[:int(len(bot_categories)/5)]*2
...
def createAndFillFile(fileName, entry):
...
def do_generate(fileName):
    ...
    for entry in generate_log(args, datetime.now()):
        fn = fd[0]+str(ind)+"."+ fd[1]
        createAndFillFile(fn, entry)  
        ind=ind+1  

def main(args):
    print("started with parameters :", args) 
    if args.file:
        do_generate(args.file)
